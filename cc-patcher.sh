...
init_modules() {
    ...
    register_module "context_limit" \
        ...
        'function iN\(A\)\{if\(A\.includes\("\[1m\]"\)\)return 1e6;return 200000\}' \
        'function iN(A){if(A.includes("[1m]"))return 1e6;if(process.env.CLAUDE_CONTEXT_LIMIT)return Number(process.env.CLAUDE_CONTEXT_LIMIT);return 200000}'
    ...
    register_module "security_context_clean" \
        ...
        '\$\{DTB\}' \
        '${process.env.CLAUDE_PERSISTENT_PROMPT?process.env.CLAUDE_PERSISTENT_PROMPT:DTB}' \
        '\+uW6;else Q=A.file' \
        ";else Q=A.file"
    ...
    register_module "temperature_setting" \
        ...
        'm.model\),temperature:F,system:J,tools' \
        "m.model),temperature:process.env.CLAUDE_CURRENT_MODE==='plan'\&\&process.env.CLAUDE_PLAN_TEMPERATURE?Number(process.env.CLAUDE_PLAN_TEMPERATURE):process.env.CLAUDE_TEMPERATURE?Number(process.env.CLAUDE_TEMPERATURE):F,system:J,tools" \
        '\],permissionMode:m.mode,promptCategory:Y' \
        '],permissionMode:(process.env.CLAUDE_CURRENT_MODE=m.mode,m.mode),promptCategory:Y'
}
