...
init_modules() {
    ...
    register_module "context_limit" \
        ...
        'function cM\(A\)\{if\(A\.includes\("\[1m\]"\)\)return 1e6;return 200000\}' \
        'function cM(A){if(A.includes("[1m]"))return 1e6;if(process.env.CLAUDE_CONTEXT_LIMIT)return Number(process.env.CLAUDE_CONTEXT_LIMIT);return 200000}'
    ...
    register_module "temperature_setting" \
        ...
        'd.model\),temperature:F,system:J,tools' \
        "d.model),temperature:process.env.CLAUDE_CURRENT_MODE==='plan'\&\&process.env.CLAUDE_PLAN_TEMPERATURE?Number(process.env.CLAUDE_PLAN_TEMPERATURE):process.env.CLAUDE_TEMPERATURE?Number(process.env.CLAUDE_TEMPERATURE):F,system:J,tools" \
